...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle n("gt");//nodeHandle用自己的名字,使用全局会产生影响.
    ...
    std::ifstream timestamp_file("/home/tr/aloc_ws/times.txt", std::ifstream::in);

   // std::string ground_truth_path = "results/" + sequence_number + ".txt";
    std::ifstream ground_truth_file("/home/tr/aloc_ws/00.txt", std::ifstream::in);
    ...
    ros::Rate r(10000);//不加的话只加载一次,需要循环发送
  ...
  // while ( ros::ok())
    {
        ...
        std::getline(ground_truth_file, line_pose);
        std::stringstream poseStream;
        poseStream<<line_pose;
